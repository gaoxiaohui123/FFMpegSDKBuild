...
static void reconfig_encoder(AVCodecContext *ctx, const AVFrame *frame)
{
  ...
  {
    x4->params.bmb_size = ctx->bmb_size;
    memcpy(x4->params.skip_mb, ctx->skip_mb, ctx->bmb_size);
    x4->params.ref_idx = ctx->ref_idx;
    x4->params.skip_frame = ctx->skip_frame;
    //printf("reconfig_encoder: ctx->skip_frame=%d \n", ctx->skip_frame);
    x264_encoder_reconfig(x4->enc, &x4->params);
  }
  else //if(ctx->skip_frame > 0)
  {
    x4->params.bmb_size = ctx->bmb_size;
    memcpy(x4->params.skip_mb, ctx->skip_mb, ctx->bmb_size);
    x4->params.skip_frame = ctx->skip_frame;
...
FF_ENABLE_DEPRECATION_WARNINGS
  ...
  {
    x4->params.bmb_size, avctx->bmb_size;
    memcpy(x4->params.skip_mb, avctx->skip_mb, avctx->bmb_size);
    x4->params.ref_idx = avctx->ref_idx;
    x4->params.skip_frame = avctx->skip_frame;
    printf("X264_init: avctx->skip_frame=%d \n", avctx->skip_frame);
    //x264_encoder_reconfig(x4->enc, &x4->params);
  }
