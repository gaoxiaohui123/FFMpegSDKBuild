...
static int add_audio_device(AVFormatContext *s, AVCaptureDevice *audio_device)
{
    ...
    dispatch_queue_t queue;
    printf("gxh: add_audio_device: audio_dev_input=%x \n", audio_dev_input);
    if (!audio_dev_input) {
        av_log(s, AV_LOG_ERROR, "Failed to create AV capture input device: %s\n",
               [[error localizedDescription] UTF8String]);
        printf("gxh: add_audio_device: Failed to create AV capture input device \n");
        return 1;
    }
    ...
    if ([ctx->capture_session canAddInput:audio_dev_input]) {
        ...
        printf("gxh: add_audio_device: can't add audio input to capture session \n");
        return 1;
    }
    ...
    if (!ctx->audio_output) {
        ...
        printf("gxh: add_audio_device: Failed to init AV audio output \n");
        return 1;
    }
    ...
    if ([ctx->capture_session canAddOutput:ctx->audio_output]) {
        ...
        printf("gxh: add_audio_device: adding audio output to capture session failed \n");
        return 1;
    }
    printf("gxh: add_audio_device: ok \n");
    return 0;
}
...
static int get_audio_config(AVFormatContext *s)
{
    ...
    AVStream* stream = avformat_new_stream(s, NULL);
    printf("gxh: get_audio_config: stream=%x \n", stream);
    if (!stream) {
        return 1;
    }
    printf("gxh: get_audio_config: ctx->audio_frames_captured=%x \n", ctx->audio_frames_captured);
    // Take stream info from the first frame.
    ...
    printf("gxh: get_audio_config: basic_desc=%x \n", basic_desc);
    if (!basic_desc) {
    ...
    printf("gxh: get_audio_config: ctx->audio_float=%d \n", ctx->audio_float);
    printf("gxh: get_audio_config: ctx->audio_signed_integer=%d \n", ctx->audio_signed_integer);
    printf("gxh: get_audio_config: ctx->audio_bits_per_sample=%d \n", ctx->audio_bits_per_sample);
    printf("gxh: get_audio_config: ctx->audio_float=%d \n", ctx->audio_float);
    printf("gxh: get_audio_config: ctx->audio_packed=%d \n", ctx->audio_packed);
    printf("gxh: get_audio_config: ctx->audio_be=%d \n", ctx->audio_be);
    ...
    //    printf("gxh: get_audio_config: added by gxh \n");
    } else if (basic_desc->mFormatID == kAudioFormatLinearPCM &&
        ctx->audio_signed_integer &&
        ctx->audio_bits_per_sample == 32 &&
        ctx->audio_packed) {
        ...
        printf("gxh: get_audio_config: kAudioFormatLinearPCM=%d \n", kAudioFormatLinearPCM);
        printf("gxh: get_audio_config: basic_desc->mFormatID=%d \n", basic_desc->mFormatID);
        printf("gxh: get_audio_config: audio format is not supported \n");
        return 1;
    }
...
static int avf_read_header(AVFormatContext *s)
{
    ...
    printf("gxh: avf_read_header: start \n");
    ...
    printf("gxh: avf_read_header: start get audio device: ctx->audio_device_index=%d, ctx->video_device_index=%d \n", ctx->audio_device_index, ctx->video_device_index);
    // get audio device
    ...
    if (!video_device && !audio_device) {
        ...
        printf("gxh: avf_read_header: No AV capture device found \n");
        goto fail;
    }
    ...
    if (audio_device && get_audio_config(s)) {
        printf("gxh: avf_read_header: get_audio_config: fail \n");
        goto fail;
    }
