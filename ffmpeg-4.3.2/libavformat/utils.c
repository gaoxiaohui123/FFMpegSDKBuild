                        ...
                        ff_const59 AVInputFormat *fmt, AVDictionary **options)
{
    ...
    printf("gxh: avformat_open_input: start \n");
    ...
    if (s->pb) // must be before any goto fail
        s->flags |= AVFMT_FLAG_CUSTOM_IO;
    printf("gxh: avformat_open_input: 1 \n");
    if ((ret = av_opt_set_dict(s, &tmp)) < 0)
        goto fail;
    printf("gxh: avformat_open_input: 2 \n");
    if (!(s->url = av_strdup(filename ? filename : ""))) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    printf("gxh: avformat_open_input: 3 \n");
#if FF_API_FORMAT_FILENAME
    printf("gxh: avformat_open_input: 4 \n");
FF_DISABLE_DEPRECATION_WARNINGS
    av_strlcpy(s->filename, filename ? filename : "", sizeof(s->filename));
FF_ENABLE_DEPRECATION_WARNINGS
#endif
    printf("gxh: avformat_open_input: 5 \n");
    if ((ret = init_input(s, filename, &tmp)) < 0)
        goto fail;
    s->probe_score = ret;
    printf("gxh: avformat_open_input: 6 \n");
    ...
    printf("gxh: avformat_open_input: 7 \n");
    ...
    printf("gxh: avformat_open_input: 8 \n");
    ...
    printf("gxh: avformat_open_input: 9 \n");
    ...
    printf("gxh: avformat_open_input: 10 \n");
    s->duration = s->start_time = AV_NOPTS_VALUE;

    /* Allocate private data. */
    if (s->iformat->priv_data_size > 0) {
        printf("gxh: avformat_open_input: 11 \n");
        if (!(s->priv_data = av_mallocz(s->iformat->priv_data_size))) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        printf("gxh: avformat_open_input: 12 \n");
    ...
    printf("gxh: avformat_open_input: 13 \n");
    ...
    if (!(s->flags&AVFMT_FLAG_PRIV_OPT) && s->iformat->read_header)
        if ((ret = s->iformat->read_header(s)) < 0)
            goto fail;
    printf("gxh: avformat_open_input: 14 \n");
    if (!s->metadata) {
    ...
    printf("gxh: avformat_open_input: 15 \n");
    ...
    *ps = s;
    printf("gxh: avformat_open_input: ok \n");
    return 0;
...
fail:
    ...
    *ps = NULL;
    printf("gxh: avformat_open_input: fail \n");
    return ret;
}
