...
int x264_ratecontrol_new( x264_t *h )
{
...
#if 1
    {
        ...
        fprintf(fp,"gxh: x264_ratecontrol_new: h->param.rc.i_rc_method = %d \n", h->param.rc.i_rc_method );
        fprintf(fp,"gxh: x264_ratecontrol_new: rc->b_abr = %d \n", rc->b_abr );
        fflush(fp);
        fclose(fp);
    }
#endif
...
void x264_ratecontrol_start( x264_t *h, int i_force_qp, int overhead )
{
    ...
    {
        FILE *fp = fopen("x264_gxh.txt", "ab+");
        fprintf(fp,"gxh: x264_ratecontrol_start: q = %f \n", q );
        //fprintf(fp,"gxh: x264_ratecontrol_start: rc->b_abr = %d \n", rc->b_abr );
        fflush(fp);
        fclose(fp);
    }
#endif
