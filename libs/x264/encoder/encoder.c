...
#define COPY(var) h->param.var = param->var
    ...
    h->param.bmb_size = param->bmb_size;
    memcpy(h->param.skip_mb, param->skip_mb, param->bmb_size);
    COPY( ref_idx );
    COPY( skip_frame );
    //printf("x264_encoder_try_reconfig: param->ref_idx=%d \n", param->ref_idx);
    //printf("x264_encoder_try_reconfig: param->skip_frame=%d \n", param->skip_frame);
#if 0
...
#define BS_BAK_ROW_VBV        3
    ...
    while( 1 )
    {
        ...
        {
#ifndef GXH
            x264_macroblock_analyse( h ,i_mb_x, i_mb_y);
#else
            x264_macroblock_analyse( h );//(i_mb_x, i_mb_y)
#endif
        }
