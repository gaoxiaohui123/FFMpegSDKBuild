...
//HCSVC_API
int x264_get_function(void **handle)
{
    int ret = 0;
    unsigned int cpu = x264_cpu_detect();
    x264_t *h;
    CHECKED_MALLOCZERO( h, sizeof(x264_t) );
    //h = x264_malloc(sizeof(x264_t));
    x264_pixel_init( cpu, &h->pixf );
    (*handle) = h;
    return ret;
fail:
    ret = -1;
...
//HCSVC_API
void x264_hnd_free(void *handle)
{
...
//HCSVC_API
int x264_pixel_sa8d_16x16_gxh(void *handle, unsigned char *src, int pitch0, unsigned char *dst, int pitch1)
{
    x264_t *h = (x264_t *)handle;
    int ret = h->pixf.sa8d[PIXEL_16x16](src, pitch0, dst, pitch1);
    return ret;
}
//HCSVC_API
int x264_pixel_sa8d_8x8_gxh(void *handle,unsigned char *src, int pitch0, unsigned char *dst, int pitch1)
{
    x264_t *h = (x264_t *)handle;
    int ret = h->pixf.sa8d[PIXEL_8x8](src, pitch0, dst, pitch1);
    return ret;
}
#endif
